...
public class ServiceRepository {
    ...
    public Future<Boolean> init() {
        ...
        connector.query("SELECT * FROM service;").setHandler(asyncResult -> {
            if (asyncResult.succeeded()) {
                asyncResult.result().getRows()
                    .forEach(row -> services.put(row.getString("url"), row.put("status", "UNKNOWN")));
    ...
    public Future<ResultSet> remove(String service) {
        ...
        return connector.query("DELETE FROM service WHERE name=?", new JsonArray().add(service));
    }
